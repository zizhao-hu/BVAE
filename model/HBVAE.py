...
class HBVAE(ConvVAE):
    ...
    def loss(self, x, reconstruction, mu, prior_mu=0):
        ...
        var_loss = 100*nn.BCELoss(reduction='sum')(mu, prior_mu)
        print(var_loss)
    ...
    def forward(self, x):
        ...
        sample_mu = 0.1*est_mu_01+0.9*pri_mu
        print(torch.max(est_mu_01))
        print(torch.min(est_mu_01))
        print(torch.max(sample_mu))
        print(torch.min(sample_mu))
